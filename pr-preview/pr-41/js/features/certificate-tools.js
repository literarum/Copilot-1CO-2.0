...
async function emulateRevocationStatus({ serialNumberHex, ocspUrls, crlUrls, timeoutMs }) {
    ...
    return {
        ...
        endpoint: DEFAULT_EMULATOR_ENDPOINT,
        message:
            'Эмулятор backend не обнаружил признак отзыва в локальном mock-списке. Внешние OCSP/CRL точки недоступны из текущей среды, статус помечен как предварительный.',
        details: {
            checkedUrls: checks,
            confidence: 'low',
            warning:
