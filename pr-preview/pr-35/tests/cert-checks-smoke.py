    ...
    async with async_playwright() as p:
        ...
        await page.set_input_files(
            "#certFileInput",
            {
                ...
                "buffer": TEST_CERT_PEM.encode("utf-8"),
            },
        )
        ...
        assert drop_hidden, "Drop-zone should be hidden after file selection"
        assert loaded_visible, "Loaded-state banner should be visible after file selection"
        assert "Ошибка проверки сертификата" not in summary, "Validation must not end with generic parse error"
