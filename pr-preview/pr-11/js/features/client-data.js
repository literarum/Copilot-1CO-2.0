...
export function ensureInnPreviewStyles() {
    ...
    #clientNotes::selection{
        background: #fef08a;
        color: inherit;
    }
    .client-notes-scroll-btn{
        position: absolute;
        right: 12px;
        bottom: 12px;
        width: 32px;
    ...
    .dark .client-notes-scroll-btn{
        background: rgba(31,41,55,0.9);
        color: #e5e7eb;
        border-color: rgba(255,255,255,0.12);
    }
    .client-notes-scroll-btn:hover{
...
function setupClientNotesScrollButton(textarea) {
    const wrapper = textarea.parentElement;
    if (!wrapper || textarea._scrollButtonWired) return;
    textarea._scrollButtonWired = true;
    ...
    button.type = 'button';
    button.className = 'client-notes-scroll-btn';
    button.setAttribute('aria-label', 'Прокрутить вниз');
    button.innerHTML = '<i class="fas fa-arrow-down"></i>';
    wrapper.appendChild(button);

    const updateButtonVisibility = () => {
        const atBottom =
            Math.ceil(textarea.scrollTop + textarea.clientHeight) >=
            textarea.scrollHeight - 4;
        button.style.display = atBottom ? 'none' : 'inline-flex';
    };
    ...
    textarea.addEventListener('scroll', updateButtonVisibility);
    textarea.addEventListener('input', updateButtonVisibility);
    textarea._updateScrollButtonVisibility = updateButtonVisibility;
    updateButtonVisibility();
}
...
export async function initClientDataSystem() {
    ...
    setupClientNotesScrollButton(clientNotes);
    ...
    try {
        ...
        if (typeof clientNotes._updateScrollButtonVisibility === 'function') {
            clientNotes._updateScrollButtonVisibility();
        }
    } catch (error) {
