# Copilot 1СО 2.0 - Система поддержки 1С
<!-- trigger deploy -->
...
### Проверка сертификатов .cer

Добавлена отдельная страница для надежной нормализации и парсинга сертификатов: `site/cert-checker.html` (после запуска сервера: `http://localhost:8000/cert-checker.html`).

Что реализовано:
- автоматическое распознавание форматов **DER / PEM / base64-only**;
- защита от ложной обработки PKCS#7 как X.509;
- безопасное прекращение обработки при ошибке `readCertHex`;
- извлечение AIA/CDP URL (OCSP/CRL) с пометкой внутренних адресов;
- встроенная эмуляция backend для GitHub Pages (`mock://revocation-local`) + опциональный вызов реального backend endpoint для криптографически корректной проверки отзыва (рекомендуется для ГОСТ).
