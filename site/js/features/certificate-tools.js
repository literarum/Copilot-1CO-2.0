...
export function normalizeCertificateInput(fileBytes) {
    ...
    const isPem = trimmed.includes('-----BEGIN CERTIFICATE-----') || trimmed.includes('-----BEGIN X509 CERTIFICATE-----');
...
export async function checkRevocationStatus({
    ...
    if (!endpoint) {
        return {
            ...
            source: 'client-only',
            message:
                'Надежная проверка отзыва требует backend/native слоя с поддержкой ГОСТ-подписей CRL/OCSP.',
        };
    }
