...
function initCertificateChecksSystem() {
    ...
    const parseCertificate = async (file) => {
        ...
        // Приоритет: DER-парсинг для .cer/.crt/.der, чтобы избежать ошибок от текстового чтения бинарника
        if (isDerByExt && !isPemByExt) {
            const buffer = await fileToArrayBuffer(file);
            certHex = arrayBufferToHex(buffer);
            certPem = base64ToPem(arrayBufferToBase64(buffer));
        } else {
            const rawText = await file.text();
            const hasPemMarkers = /-----BEGIN CERTIFICATE-----/.test(rawText);

            if (isPemByExt || hasPemMarkers) {
                certPem = rawText;
                certHex = derHexFromPem(rawText);
            } else {
                const buffer = await fileToArrayBuffer(file);
                certHex = arrayBufferToHex(buffer);
                certPem = base64ToPem(arrayBufferToBase64(buffer));
        ...
        const x509 = new X509();
        try {
            if (certPem && typeof x509.readCertPEM === 'function') {
