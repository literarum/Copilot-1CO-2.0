...
function initCertificateChecksSystem() {
    ...
    const isLikelyBase64CertBody = (text) => {
        const normalized = (text || '').replace(/\s+/g, '');
        if (!normalized || normalized.length < 64) return false;
        if (!/^[A-Za-z0-9+/=]+$/.test(normalized)) return false;
        return normalized.startsWith('MII');
    };
    ...
    const parseCertificate = async (file) => {
        ...
        // Некоторые .cer/.crt приходят как PEM-текст или как base64-only (без BEGIN/END)
        const asText = new TextDecoder('utf-8', { fatal: false }).decode(buffer);
        const hasPemMarkers = /-----BEGIN CERTIFICATE-----/.test(asText);
        const hasBase64OnlyBody = !hasPemMarkers && isLikelyBase64CertBody(asText);

        if ((isDerByExt && !hasPemMarkers && !hasBase64OnlyBody) || (!isPemByExt && !hasPemMarkers && !hasBase64OnlyBody)) {
            certHex = arrayBufferToHex(buffer);
            certPem = base64ToPem(arrayBufferToBase64(buffer));
        } else if (hasBase64OnlyBody) {
            certPem = base64ToPem(asText.replace(/\s+/g, ''));
            certHex = derHexFromPem(certPem);
        } else {
            certPem = asText;
        ...
        try {
            ...
            console.log('[certChecks] X509 readCertHex failed, trying ASN.1 fallback:', error);
